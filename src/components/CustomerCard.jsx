...
const CustomerCard = ({ customer, onTransactionClick, onCustomerUpdated }) => {
  ...
  const handleSendSMS = () => {
    const defaultMessage = `مرحبا ${
      customer.name
    }،\n\nرصيدك الحالي: ${formatCurrency(Math.abs(balance))} شيكل ${
      balance >= 0 ? "دين" : "رصيد"
    }\n\nشكراً لتعاملك معنا`;
  ...
  const handleSMSConfirm = (result) => {
    ...
    if (result.success) {
      ...
      setToast({ message: "✗ فشل إرسال الرسالة", type: "error" });
    }
  };
          ...
          >
            ...
            <div style={{ padding: "var(--spacing-md) 0" }}>
              ...
              <div className="alert alert-error">
                ⚠️ سيتم حذف جميع المعاملات المرتبطة بهذا الزبون (
